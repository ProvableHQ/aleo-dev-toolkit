...
`ShieldWalletAdapter` pairs with the Shield app from a plain mobile browser (no extension) by default. Production relay URL, deeplink, and transport are built in — pass a config object only to override them for testing, or `{ remote: false }` for injected-only behavior. See the [Shield adapter README](https://github.com/ProvableHQ/aleo-dev-toolkit/blob/master/packages/aleo-wallet-adapter/wallets/shield/README.md).
...
- **`decryptPermission`** (optional): Decrypt permission level. Options:
  ...
  - `DecryptPermission.OnChainHistory` - The dapp can request on-chain record plain texts and transaction ids and also decrypt any requested records
