...
export class PuzzleWalletAdapter extends BaseAleoWalletAdapter {
  ...
  async connect(): Promise<Account> {
    try {
      ...
      // Get address from the response using type casting
      // This handles differences in the Puzzle SDK response structure
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      this.emit('error', err);
      throw new WalletDisconnectionError(err.message || 'Disconnection failed');
    }
  }
