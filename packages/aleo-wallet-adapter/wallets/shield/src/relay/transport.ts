/**
 * VENDORED from ProvableHQ/shield-relay@1c390ae (packages/dapp-client/src/transport.ts).
...
export class RemoteShieldTransport {
  ...
  private handlePayload(payload: RelayPayload): void {
    if (payload.type === 'key_handshake') {
      ...
      // Key pinning: the FIRST authenticated key owns the pairing for its
      // lifetime. A later valid-MAC handshake with a different key is someone
      // who obtained the connect URL + secret trying to swap themselves in —
      // never rebind an established session. (Same key re-announcing itself,
      // e.g. a restored wallet, is idempotent and fine.)
      if (this.walletPublicKey && this.walletPublicKey !== payload.publicKey) {
        this.emit('handshakeRejected');
        return;
      }
