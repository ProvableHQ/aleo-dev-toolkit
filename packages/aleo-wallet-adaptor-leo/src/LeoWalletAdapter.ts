...
export class LeoWalletAdapter extends BaseAleoWalletAdapter {
  ...
  private _checkAvailability(): void {
    if (typeof window === 'undefined' || typeof window.leoWallet === 'undefined') {
      this.readyState = WalletReadyState.UNSUPPORTED;
      return;
    }
    
    this._provider = window.leoWallet;
  ...
  async connect(): Promise<Account> {
    ...
    if (!this._provider) {
      throw new WalletConnectionError('Leo Wallet provider not found');
    }
