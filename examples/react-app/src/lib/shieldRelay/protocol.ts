/**
 * VENDORED from ProvableHQ/shield-relay@1c390ae (packages/protocol/src: types.ts + crypto.ts + connectUrl.ts).
...
import type { ConnectRequest } from './connectUrl.js';
 ...
 *
 * On the http(s) form the params travel in the FRAGMENT, never the query
 * string: when the app is not installed the OS falls back to loading the
 * page, and a query string would send the bearer pairing `secret` to the web
 * server — access logs, analytics, browser history. Fragments never leave
 * the browser, and the fallback page can still read them in JS to render
 * "dapp.example wants to connect". Every param moves (not just `secret`) so
 * there is one rule per URL form instead of a split parser. Custom schemes
 * keep the query string — they never reach a server — and parseConnectUrl
 * accepts both forms, so both clients stay in lockstep.
 */
...
export function buildConnectUrl(base: string, req: Omit<ConnectRequest, 'version'>): string {
  ...
  params.set('channelId', req.channelId);
  params.set('pubkey', req.publicKey);
  params.set('relay', req.relay);
  params.set('origin', req.origin);
  params.set('secret', req.secret);
  params.set('v', String(PROTOCOL_VERSION));
  if (url.protocol === 'http:' || url.protocol === 'https:') {
    // Universal links: the fragment is never sent to the server hosting the
    // fallback page, so the pairing secret cannot leak into logs/analytics.
    url.hash = params.toString();
  } else {
    // Custom schemes never reach a server; the query form stays.
...
export function parseConnectUrl(raw: string): ConnectRequest {
  const url = new URL(raw);
  // Fragment form (http(s) universal links) or query form (custom schemes).
