...
async function getCurrentNetworkState() {
  try {
    ...
    if (!heightResponse.ok || !stateRootResponse.ok) {
      throw new Error(
        `API request failed: height=${heightResponse.status}, stateRoot=${stateRootResponse.status}`,
      );
    }
...
async function buildProvingRequest(
  ...
  try {
    ...
    const authorizationObj = executionAuthorization
      ? JSON.parse(executionAuthorization.toString())
      : null;
    ...
    if (error.message.includes('buildAuthorization')) {
      throw new Error(
        `Failed to build authorization with v0.9.3 API: ${error.message}. Make sure you're using the correct SDK version.`,
      );
    }
...
async function localProgramExecutionWithFee(
  ...
  const keySearchParams = new SDK.AleoKeyProviderParams({
  ...
  let outputs = [];
  try {
    outputs = JSON.parse(txString).transitions?.[0]?.outputs ?? [];
  } catch {
    /* noop – leave outputs as [] */
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 529) {
        console.warn(
          '⚠️ Broadcast service temporarily unavailable (529). Transaction generated but not broadcast.',
        );
...
async function buildDelegatedProvingRequest(
  program_source,
  aleoFunction,
  inputs,
  privateKey,
  ...
  const requestData = await buildProvingRequest(
    program_source,
    aleoFunction,
    inputs,
    privateKey,
...
async function executeDelegatedProvingRequest(requestData, provingServiceUrl, broadcast = true) {
  ...
  if (result.transaction) {
    ...
    execution =
      typeof result.execution === 'string' ? result.execution : JSON.stringify(result.execution);
    try {
      const executionObj =
        typeof result.execution === 'string' ? JSON.parse(result.execution) : result.execution;
  ...
  console.log(
    `✅ Delegated proving request execution completed with v0.9.3 (broadcast=${broadcast})`,
  );
